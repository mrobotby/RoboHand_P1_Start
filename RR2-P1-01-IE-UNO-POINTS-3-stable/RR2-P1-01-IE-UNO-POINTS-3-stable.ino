...
double y_enter = 0;
double z_enter = 0;
double gripper_open_close_enter = 0;
...
label1:
  ...
  //Блок Открыть-Закрыть гриппер. Если 0, то гриппер будет открыт, если 1, то гриппер будет закрыт.
  Serial.println("Enter 0 to open Gripper or 1 to close Gripper. Press ENTER");
  while (Serial.available() == 0) ;
  {
    gripper_open_close_enter = Serial.parseFloat();
    Serial.print("Gripper Open 0 or Close 1 = "); Serial.println(gripper_open_close_enter, DEC);
    while (Serial.available() > 0)
    {
      junk = Serial.read() ;
    }

    //Закомментировать одну строку ниже, если указывается угол наклона гриппера далее.
    gripper_enter = 0;
  }

  //Раскомментировать блок, если необходимо указывать угол наклона гриппера.
  //Может понадобится, когда предмет надо брать сверху и т.д.
  //Не забудь закомментировать строку  gripper_enter = 0; выше
  ...
  */
  ...
  while (Serial.available() == 0) ;
  {
    ...
    if (right_enter == 1)
    {
      ...
      Serial.println(gripper_open_close_enter);
      set_arm(x_enter, y_enter, z_enter, gripper_enter);// Опускаем захват, ставя точку.
      //Теперь открываем или закрываем гриппер, в зависимости от поданной ранее команды.
      if (gripper_open_close_enter == 0) {
        //Открываем
        servo_gripper.writeMicroseconds(gripper_servopulse_closed);
      }
      if (gripper_open_close_enter == 1) {
       //Закрываем
        servo_gripper.writeMicroseconds(gripper_servopulse_open);
    }
  }
  else
  {
